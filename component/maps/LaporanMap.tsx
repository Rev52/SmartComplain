...
interface LaporanMapProps {
    laporan: LaporanItem[]
    // FIX: Terima prop loading dari parent agar peta bisa
    // langsung di-mount paralel dengan fetch data
    loading?: boolean
}
...
// FIX: Cache icon per prioritas di level module —
// dibuat sekali saat file diload, tidak re-create tiap render
...
function MarkerPopup({ item }: { item: LaporanItem }) {
    ...
    const pillBase: React.CSSProperties = { fontSize: "11px", padding: "2px 8px", borderRadius: "999px", fontWeight: 500 }

    return (
        <div style={{ minWidth: "180px" }}>
            <p style={{ fontWeight: 600, marginBottom: "4px", fontSize: "13px" }}>{item.judul}</p>
            <p style={{ ...baseText }}>{item.kategori} · {item.kecamatan ?? "-"}</p>
            <p style={{ ...baseText, marginBottom: "8px" }}>{item.lokasi ?? "-"}</p>
            <div style={{ display: "flex", gap: "6px" }}>
                <span style={{ ...pillBase, background: badge?.bg ?? "#f3f4f6", color: badge?.color ?? "#374151" }}>
                    {p_label}
                </span>
...
export default function LaporanMap({ laporan, loading }: LaporanMapProps) {
    return (
        // FIX: MapContainer selalu di-render (tidak tunggu loading),
        // sehingga Leaflet tiles mulai diload paralel dengan data Supabase.
        // Marker akan muncul begitu data datang tanpa reload peta.
        <div style={{ position: "relative" }}>
            ...
            >
                ...
                {laporan.map(item => (
                    <Marker
                        ...
                        // FIX: Ambil dari cache, tidak bikin object baru tiap render
            ...
            {/* FIX: Overlay tipis saat data masih loading,
                peta tetap tampil dan tile sudah ter-load */}
            {loading && (
                <div style={{
