...
export default async function AdminBerandaPage() {
  ...
  const ditindaklanjuti =
    laporan?.filter((l) => l.status === "ditindaklanjuti").length ?? 0;
  const ditolak = laporan?.filter((l) => l.status === "ditolak").length ?? 0;
  const prioritasTinggi =
    laporan?.filter((l) => l.prioritas === "tinggi").length ?? 0;
  const prioritasSedang =
    laporan?.filter((l) => l.prioritas === "sedang").length ?? 0;
  const prioritasRendah =
    laporan?.filter((l) => l.prioritas === "rendah").length ?? 0;
              ...
              <div className="px-6 pb-5">
                <table className="admin-table">
                  ...
                  <tbody>
                    {[
                      {
                        label: "1. Prioritas Tinggi",
                        value: prioritasTinggi,
                        color: "text-red-500",
                      },
                      {
                        label: "2. Prioritas Sedang",
                        value: prioritasSedang,
                        color: "text-yellow-500",
                      },
                      {
                        label: "3. Prioritas Rendah",
